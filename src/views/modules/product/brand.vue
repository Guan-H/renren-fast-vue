    ...
    >
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="250" label="操作">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="deleteHandle(scope.row.brandId,scope.row.dataList.name)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    // 删除
    deleteHandle(id,namees) {
      var names = this.dataList.name 
      ?[this.dataList.name]
      :this.dataListSelections.map(item => {
            return item.name;
        });
      console.log("name:",this.namees)
      console.log(this)
      ...
      this.$confirm(
        `确定对[${ids.join(",")}]进行[${namees ? "删除" : "批量删除"}]操作?`,
        "提示",
        {
